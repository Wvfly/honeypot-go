# build-win.ps1 编译 honeypot / ttyshow / dbquery / anti_attack 到 Windows 本机，并校验产物为 PE。
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
    go build -trimpath -ldflags "-s -w" -o "target/anti_attack-windows-$Arch.exe" ./cmd/anti_attack
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
}
finally {
    # 恢复环境变量，避免影响后续交叉编译
...
foreach ($name in @("target/honeypot-windows-$Arch.exe", "target/ttyshow-windows-$Arch.exe", "target/dbquery-windows-$Arch.exe", "target/anti_attack-windows-$Arch.exe")) {
